  ...
  <Layout>
    <div style="min-height: 600px" v-loading="loading">
      <el-card
        ...
        v-if="$page.allStrapiPosts.edges[0].node.id"
      >
        <div slot="header">
          <span>{{ $page.allStrapiPosts.edges[0].node.title }}</span>
        </div>
        <div style="font-size: 0.9rem; line-height: 1.5; color: #606c71">
          发布 {{ $page.allStrapiPosts.edges[0].node.createTime }} <br />
          更新 {{ $page.allStrapiPosts.edges[0].node.updateTime }}
        </div>
        <div
          style="
            font-size: 1.1rem;
        ...
        >
          <pre style="font-family: '微软雅黑'">{{
            $page.allStrapiPosts.edges[0].node.description
          }}</pre>
        </div>
        <div
          v-html="blog.content"
...
export default {
  ...
  mounted() {
    this.blog.content = this.$markdown(
      this.$page.allStrapiPosts.edges[0].node.content
    );
    // this.loading = true;
